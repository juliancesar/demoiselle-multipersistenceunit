package br.com.juliancesar.multipersistenceunit.service;

import java.util.List;

import javax.inject.Inject;
import javax.transaction.Transactional;
import javax.ws.rs.Consumes;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.Path;
import javax.ws.rs.Produces;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;

import br.com.juliancesar.multipersistenceunit.business.NewsCategoryBusiness;
import br.com.juliancesar.multipersistenceunit.entity.NewsCategory;

@Path("newsCategory")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class NewsCategoryREST {

	@Inject
	private NewsCategoryBusiness business;

	@POST
	@Transactional
	public Response create(NewsCategory noticia) {
		business.create(noticia);
		return Response.ok().build();
	}

	@GET
	public List<NewsCategory> list() {
		return business.list();
	}

}
